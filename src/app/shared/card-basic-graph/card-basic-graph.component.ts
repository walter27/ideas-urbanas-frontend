...
export class CardBasicGraphComponent implements OnInit, OnChanges {
    ...
    tooltips: {
      ...
      custom(tooltip) {
        ...
        if (tooltip.body) {
          ...
          const measure_symbol = afterBody.length > 1 && afterBody[1] != undefined ? ' ' + afterBody[1] : '';
  ...
  getCities() {
    if (this.citySelected) {
      ...
      this.regionService.listRegionsPublic(this.filters, 'Canton').subscribe(cities => {
        cities.data.forEach(c => {
          this.cities.push({ id: c._id, name: c.name, check: true, color: c.color });
        });
        this.getData();
      });
    }
              ...
              this.resultData.forEach(d => {
                if (d.obj_Canton._id === c.id && d.year.toString() === y) {
                  this.lineChartData[
                    ...
                    = { x: d.description, y: d.value, t: this.variableSelected.measure_symbol };
                }
              });
            });
          }
